...
for i in range(52): #generar matriz de Vigenere con alfabteto de mayúsculas y minúsculas
    ...
    for indx in range(65,65+offset2):
        MPendientes.append(chr(indx)) #las mayúsculas que no se hayan puesto, se guardan para hacerlo después
...
for keyChar in expandedKey:
    i=0
    encontrado=False
    while i<len(matrix[0]) and not encontrado:
        if matrix[Values[keyChar].value][i]==cadena[count]: #se busca en la fila de la clave, la letra del mensaje
            encontrado=True
        else:
            i+=1

    decoded=str(decoded+str(Values(i).name))
    count+=1
